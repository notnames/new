	...
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="攻略分类" prop="gonglvefenlei">
					<el-select :disabled="ro.gonglvefenlei" v-model="ruleForm.gonglvefenlei" placeholder="请选择攻略分类" >
						<el-option
							v-for="(item,index) in gonglvefenleiOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="攻略分类" prop="gonglvefenlei">
					<el-input v-model="ruleForm.gonglvefenlei"
						placeholder="攻略分类" readonly></el-input>
				</el-form-item>
			</template>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-if="type!='info'"  label="攻略详情" prop="gonglvexiangqing">
					<editor 
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else-if="ruleForm.gonglvexiangqing" label="攻略详情" prop="gonglvexiangqing">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#000","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.gonglvexiangqing"></span>
                </el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='gonglvefenlei'){
							this.ruleForm.gonglvefenlei = obj[o];
							this.ro.gonglvefenlei = true;
							continue;
						}
            ...
            this.$http({
				url: `option/gonglvefenlei/gonglvefenlei`,
				method: "get"
            }).then(({ data }) => {
				if (data && data.code === 0) {
					this.gonglvefenleiOptions = data.data;
				} else {
					this.$message.error(data.msg);
				}
            });
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.gonglvexiangqing = this.ruleForm.gonglvexiangqing.replace(reg,'../../../springbootu1xmu/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `gonglvelan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.gonglvelanCrossAddOrUpdateFlag = false;
						 this.parent.search();
