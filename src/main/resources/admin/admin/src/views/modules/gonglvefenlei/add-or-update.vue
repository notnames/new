<template>
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="攻略分类" prop="gonglvefenlei">
					<el-input v-model="ruleForm.gonglvefenlei" placeholder="攻略分类" clearable  :readonly="ro.gonglvefenlei"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="攻略分类" prop="gonglvefenlei">
					<el-input v-model="ruleForm.gonglvefenlei" placeholder="攻略分类" readonly></el-input>
				</el-form-item>
			</template>
...
export default {
	data() {
		...
		return {
			...
			rules: {
				gonglvefenlei: [
					{ required: true, message: '攻略分类不能为空', trigger: 'blur' },
				],
			}
		};
	},
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `gonglvefenlei/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.gonglvefenleiCrossAddOrUpdateFlag = false;
						 this.parent.search();
